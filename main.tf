...
resource aws_iam_policy s3_mc_server_policy {
  ...
  policy = jsondecode(filepath("${path.module}/policies/s3_mc_server_policy.json"))
}
...
resource aws_iam_policy ssm_ec2_mc_server_policy {
  ...
  policy = jsondecode(filepath("${path.module}/policies/ssm_ec2_mc_server_policy.json"))
}
...
resource aws_iam_policy iam_cloudwatch_roles_ec2_mc_server_policy {
  ...
  policy = jsondecode(filepath("${path.module}/policies/iam_cloudwatch_roles_ec2_mc_server_policy.json"))
}
